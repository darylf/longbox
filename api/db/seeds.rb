...
when 'development'
  ...
  def create_books_for(series)
    ...
    (0..total_books).each do |x|
      book = Book.create(issue: x, book_type: 1, series_id: series.id)
      BookCreator.create(book_id: book.id, creator_id: writer.id, role: 1)
      BookCreator.create(book_id: book.id, creator_id: artist.id, role: 2)
    end
  end
